...
export class DemoProvider implements Provider {
  ...
  async chat(opts: ChatOptions): Promise<ProviderResponse> {
    ...
    const userText = last?.role === "user"
      ? last.content.filter((b) => b.type === "text").map((b: ContentBlock) => (b as { text: string }).text).join(" ")
      : "";
      ...
      content: [
        {
          type: "text",
          text: userText
            ? `[provider démo] J'ai bien reçu : « ${userText.slice(0, 140)}${
                userText.length > 140 ? "…" : ""
              } ». Essaie « lis README.md », « exécute \`ls\` », « skill hello », ou « agent explorer ».`
