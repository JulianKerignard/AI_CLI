...
export function builtinCommands(allCommands: () => SlashCommand[]): SlashCommand[] {
  return [
    ...
    },
    {
      name: "paste",
      description:
        "Attache l'image du clipboard système (screenshot Cmd+Shift+4, copy image). Marche sur macOS/Linux/Windows.",
      async run() {
        const { pasteFromClipboard } = await import(
        ...
        try {
          ...
          log.info(
            `Image clipboard attachée : ${item.displayName} (${Math.round(item.sizeBytes / 1024)}k). Envoie ton prompt.`,
          );
        } catch (err) {
    ...
    },
    {
      name: "image",
      description:
        "Attache une image au prochain message (png/jpg/webp/gif, max 4.5M, 4 images max). Usage : /image ./screenshot.png",
      async run(_ctx, args) {
        ...
        if (!path) {
          ...
          log.info(
            `${items.length} image(s) en attente :\n` +
              items
                .map(
                  (i) =>
                    `  • ${i.displayName} (${Math.round(i.sizeBytes / 1024)}k)`,
                )
                .join("\n"),
          );
          return;
        ...
        try {
          ...
          log.info(
            `Image attachée : ${item.displayName} (${Math.round(item.sizeBytes / 1024)}k). Envoie ton prompt.`,
          );
        } catch (err) {
          log.error((err as Error).message);
        }
    ...
    },
    {
      name: "refresh",
      description:
        "Force le refresh du catalogue de modèles + latences (bypass cache 30s).",
      async run(ctx) {
        const { invalidateCatalog } = await import(
    ...
    },
    {
      name: "mode",
      description:
        "Cycle permission mode (alternative à Shift+Tab pour conhost/cmd.exe).",
      async run({ permissions }) {
        const { inputController } = await import(
