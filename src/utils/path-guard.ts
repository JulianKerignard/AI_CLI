...
export function guardPath(absPath: string, opts: GuardOpts): void {
  if (process.env.AICLI_ALLOW_UNSAFE_PATHS === "1") return;

  const deny = denyList();
  for (const denied of deny) {
    if (absPath === denied || absPath.startsWith(denied + sep)) {
      throw new Error(
        `Accès refusé : ${absPath} est dans la denylist (secrets système). ` +
  ...
  if (SECRET_BASENAMES.has(basename)) {
    const inCwd =
      absPath === opts.cwd || absPath.startsWith(opts.cwd + sep);
    if (!inCwd) {
      throw new Error(
        `Accès refusé : ${basename} en dehors du cwd est suspect (secret). ` +
          `Set AICLI_ALLOW_UNSAFE_PATHS=1 pour bypass.`,
      );
    }
  }
  ...
  if (opts.mode === "write") {
    ...
    if (!inCwd) {
      throw new Error(
        `Écriture refusée hors du cwd : ${absPath}. ` +
