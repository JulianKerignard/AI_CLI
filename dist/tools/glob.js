...
export const globTool = {
    name: "Glob",
    description: "Trouve des fichiers par pattern glob (ex: '**/*.ts', 'src/**/*.{ts,tsx}'). Triés par date de modification décroissante. Ignore node_modules, .git, dist, etc. par défaut.",
    ...
    formatResult: (_input, output) => {
        ...
        if (more)
            return `${lines.length}+${more[1]} matches`;
    ...
    async run(input, ctx) {
        ...
        if (!pattern)
            throw new Error("Glob: 'pattern' manquant");
        ...
        if (matches.length === 0)
            return `(aucun fichier matche ${pattern})`;
...
async function walk(current, root, regex, out, includeHidden) {
    if (out.length >= MAX_RESULTS * 2)
        return; // Hard stop pour très gros repos.
    let entries;
    try {
...
function expandBraces(pattern) {
    ...
    for (const p of parts) {
        for (const expanded of expandBraces(head + p + tail))
            out.push(expanded);
    }
    return out;
}
