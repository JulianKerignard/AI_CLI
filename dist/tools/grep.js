...
export const grepTool = {
    name: "Grep",
    description: "Recherche un pattern regex dans les fichiers. Utilise ripgrep si disponible (ignore .gitignore par défaut). Modes : files_with_matches | content | count.",
    ...
    formatResult: (input, output) => {
        ...
        if (mode === "content")
            return `${n}${more ? "+" : ""} matching lines`;
        if (mode === "count")
            return `${n} files with matches`;
        return `${n} file${n > 1 ? "s" : ""}`;
    },
    schema: {
        type: "object",
    ...
    async run(input, ctx) {
        const pattern = String(input.pattern ?? "");
        if (!pattern)
            throw new Error("Grep: 'pattern' manquant");
...
function truncateOutput(output, headLimit, mode) {
    let text = output;
...
async function fallbackGrep(opts) {
    ...
    try {
        ...
        throw new Error(`Grep: regex invalide (${err instanceof Error ? err.message : err})`);
    }
    const files = [];
    ...
    for (const file of files) {
        ...
        if (fileCount >= opts.headLimit && opts.outputMode !== "content")
            break;
    }
...
async function collectFiles(root, current, out, globPattern) {
    let entries;
    try {
    ...
    for (const entry of entries) {
        if (entry.name.startsWith(".") && entry.name !== ".")
            continue;
        if (FALLBACK_IGNORED.has(entry.name))
            continue;
        ...
        if (entry.isDirectory()) {
            ...
            if (globPattern) {
                ...
                if (!simpleGlobMatch(rel, globPattern))
                    continue;
            }
            out.push(full);
        }
