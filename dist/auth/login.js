...
import { saveCredentials, DEFAULT_BASE_URL, DEFAULT_MODEL, } from "./store.js";
    ...
    return new Promise((resolve, reject) => {
        ...
        const server = createServer((req, res) => {
            ...
            if (!token || receivedState !== state) {
                ...
                res.writeHead(400, { "Content-Type": "text/html; charset=utf-8" }).end("<h1>État invalide</h1><p>Lien périmé (un autre /login a été lancé). Relance /login et utilise le lien le plus récent, ou paste le token manuellement dans le terminal.</p>");
                log.warn("Callback reçu avec state invalide (probable /login relancé). Paste le token manuellement ou relance /login.");
                return;
            }
...
<style>body{font-family:-apple-system,system-ui,sans-serif;background:#2b2621;color:#f6f1e8;display:flex;min-height:100vh;align-items:center;justify-content:center;margin:0}.c{text-align:center;padding:40px;max-width:420px}.t{color:#e27649;font-size:18px;margin-bottom:10px}.d{color:#bdb3a1;font-size:14px;line-height:1.5}</style>
      ...
      `);
        ...
        server.listen(0, "127.0.0.1", () => {
            ...
            console.log();
            log.faint("  (écoute sur 127.0.0.1:" + port + ", timeout 3 min)");
            log.faint("  Si le navigateur ne s'ouvre pas, copie-colle l'URL ci-dessus");
        ...
        void (async () => {
            try {
                ...
                const pasted = (await input({
                    message: chalk.hex("#8a8270")("paste token ou Entrée pour attendre le browser"),
                })).trim();
                if (!pasted)
                    return; // attendre le callback
