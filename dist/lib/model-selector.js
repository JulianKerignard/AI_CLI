...
function detectSpeed(description) {
    if (!description)
        return 3; // Mistral sans tag : supposé rapide
    if (/\brapide\b/i.test(description))
        return SPEED_BY_TAG.rapide;
    if (/\bmoyen\b/i.test(description))
        return SPEED_BY_TAG.moyen;
    if (/\blent\b/i.test(description))
...
export function displayModelId(id) {
...
function qualityFallback(category) {
    ...
    if (cat === "strong")
        return 7.5;
    if (cat === "code" || cat === "thinking" || cat === "reasoning")
        return 7.5;
    if (cat === "chat")
        return 6;
    if (cat === "small" || cat === "persona")
        return 4;
    return 6;
}
// Indices /10 pour affichage UI (status bar, /best). Priorité : lookup
// benchmark map → fallback par catégorie.
export function qualityOutOf10(category, id) {
    if (id) {
...
export function speedOutOf10(description) {
    if (!description)
        return 8;
    if (/\brapide\b/i.test(description))
        return 10;
...
export function scoreModel(m, mode) {
...
export function pickBest(models, mode) {
    return models
