...
export async function startRepl() {
    ...
    setSlashCommands(commands.list().map((c) => ({ name: c.name, description: c.description })));
    ...
    log.info(`${log.kicker("provider")}  ${log.ink(cleanProvider(provider.name))}   ${log.kicker("cwd")}  ${log.inkMuted(CWD)}`);
    ...
    void (async () => {
        try {
            ...
            if (status?.updateAvailable && status.latest) {
                log.info(`${log.accentSoft("↻")} mise à jour dispo (${status.channel}) : ${log.inkMuted(status.current)} → ${log.accent.bold(status.latest)} · tape ${log.accent.bold("/update")}`);
            }
        }
        catch {
