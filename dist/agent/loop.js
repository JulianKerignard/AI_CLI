...
export class AgentLoop {
    ...
    async send(userInput) {
        ...
        const { addImage, takeAllAndClear } = await import("../ui/pending-images.js");
        const imgRegex = /(?:'([^']+\.(?:png|jpe?g|webp|gif))'|"([^"]+\.(?:png|jpe?g|webp|gif))"|(\S+\.(?:png|jpe?g|webp|gif)))/gi;
        ...
        for (const path of detectedPaths) {
            try {
                ...
                log.info(`Image détectée et attachée : ${item.displayName} (${Math.round(item.sizeBytes / 1024)}k)`);
            }
            catch (err) {
                // Path existe pas ou extension invalide : laisse tel quel dans le texte.
                log.faint(`(path ${path} ignoré : ${err.message})`);
            }
        }
        if (!cleanedText)
            cleanedText = "décris cette image";
