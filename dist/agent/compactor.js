...
export function shouldCompact(messages, contextWindow) {
    if (DISABLED)
        return false;
    ...
    if (contextWindow && contextWindow > 0) {
        ...
        if (tokens * TOKEN_ESTIMATE_SAFETY > relativeLimit)
            return true;
    }
...
function extractOpenToolUseIds(tailMessages) {
    const ids = new Set();
...
export async function compactMessages(messages, provider, systemPrompt, contextWindow) {
    if (!shouldCompact(messages, contextWindow))
        return false;
    if (messages.length <= ABSOLUTE_MIN_HEAD + KEEP_TAIL_MESSAGES)
        return false;
    ...
    while (splitAt > ABSOLUTE_MIN_HEAD) {
        ...
        const lastHeadToolUseIds = new Set();
        for (const b of lastHeadMsg.content) {
            if (b.type === "tool_use")
                lastHeadToolUseIds.add(b.id);
        }
...
Format : un seul paragraphe dense, sans bullet points. Max 400 mots. Ne réponds PAS au format markdown artifact, pas de titres. Commence directement par le résumé factuel.`,
    ...
    log.faint(`[compact] résumé des ${splitAt} premiers messages (${estimateTokens(finalHead)} tokens)…`);
    ...
    log.faint(`[compact] ${splitAt} → 1 messages, ${estimateTokens(finalHead)} → ${estimateTokens([summaryMessage])} tokens.`);
    return true;
}
