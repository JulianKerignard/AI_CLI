...
export function makeAgentTool(opts) {
    return {
        name: "Agent",
        description: "Délègue une tâche à un sub-agent spécialisé. Input { name, prompt }. Retourne la réponse finale du sub-agent.",
        schema: {
            type: "object",
            properties: {
        ...
        async run(input, ctx) {
            ...
            if (!subAgent) {
                return `Sub-agent inconnu: ${name}. Disponibles: ${opts.subAgents.map((a) => a.name).join(", ") || "(aucun)"}`;
            }
